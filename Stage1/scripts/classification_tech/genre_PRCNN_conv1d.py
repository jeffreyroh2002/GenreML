...
def Parallel_CNN_RNN(input_shape):
    ...
    C_layer = keras.layers.Conv1D(filters=16, kernel_size=3, activation='relu', padding='same')(input)
    C_layer = keras.layers.BatchNormalization()(C_layer)
    C_layer = keras.layers.Dropout(0.2)(C_layer)
    C_layer = keras.layers.MaxPooling1D(pool_size=2)(C_layer)
    
    C_layer = keras.layers.Conv1D(filters=32, kernel_size=3, activation='relu', padding='same')(C_layer)
    C_layer = keras.layers.BatchNormalization()(C_layer)
    C_layer = keras.layers.Dropout(0.2)(C_layer)
    C_layer = keras.layers.MaxPooling1D(pool_size=2)(C_layer)
    
    C_layer = keras.layers.Conv1D(filters=64, kernel_size=3, activation='relu', padding='same')(C_layer)
    C_layer = keras.layers.BatchNormalization()(C_layer)
    C_layer = keras.layers.Dropout(0.2)(C_layer)
    C_layer = keras.layers.MaxPooling1D(pool_size=2)(C_layer)
    
    C_layer = keras.layers.Conv1D(filters=128, kernel_size=3, activation='relu', padding='same')(C_layer)
    C_layer = keras.layers.BatchNormalization()(C_layer)
    C_layer = keras.layers.Dropout(0.2)(C_layer)
    C_layer = keras.layers.MaxPooling1D(pool_size=2)(C_layer)
    ...
    R_layer = keras.layers.MaxPooling1D(pool_size=2)(input)
    # R_layer = keras.layers.MaxPooling2D(pool_size=(1,2), strides=(1,2))(R_layer)
    # R_layer = keras.layers.Reshape(target_shape=(128,256))(R_layer1)
    ...
    R_layer = keras.layers.Bidirectional(keras.layers.GRU(128, return_sequences=True))(R_layer)
    R_layer = keras.layers.BatchNormalization()(R_layer)
    R_layer = keras.layers.Dropout(0.5)(R_layer)
    R_layer = keras.layers.Bidirectional(keras.layers.GRU(128, return_sequences=False))(R_layer)
    R_layer = keras.layers.BatchNormalization()(R_layer)
    R_layer = keras.layers.Dropout(0.5)(R_layer)
    # R_layer = keras.layers.Bidirectional(keras.layers.GRU(256))(R_layer)
